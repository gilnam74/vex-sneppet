...
Classes are the small programs, just like functions. Imagine classes as containers for basic functions. You run class program same way as a usual function: `GeoCreator()`. If you assign the result of class execution to a variable (`win = GeoCreator()`) you create an instance of a class — object `win`. Same thing we did when create a Houdini root scene object (`sceneRoot = hou.node('/obj/')`). Then you can apply a specific command to this objects, to display UI in Houdini we use `show()` command: `win.show()`. And you need PySide libraries in your program to make UI widgets working, so we import them at the beginning: `from PySide2 import QtCore, QtUiTools, QtWidgets`. This basic template will remain the same with any other tool you will want to create, you will need just to build actual functionality on top of that. 
